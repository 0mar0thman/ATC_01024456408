...
<head>
    ...
    <title>فاتورة جديدة - {{ $data['booking_id'] }}</title>
    <style>
        @font-face {
            font-family: 'Tajawal';
...
<body>
    <div class="email-container">
        <div class="email-header">
            <h1>حجز جديد #{{ $data['booking_id'] }}</h1>
        </div>
        ...
        <div class="email-body">
            <div class="invoice-details">
                <div class="detail-row">
                    ...
                    <div class="detail-value">{{ $data['booking_id'] }}</div>
                </div>
                <div class="detail-row">
                    {{-- <img src="{{ $data['event']['image'] ? asset('Attachments/Events/' .  $data['event']['image']) : asset('images/default-event.jpg') }}"
                        style="max-width: 100%; height: auto; display: block; margin: 0 auto;"
                        alt="{{ $data['event']['title'] }}"> --}}
                </div>
                <div class="detail-row">
                    <div class="detail-label">تاريخ الحجز:</div>
                    <div class="detail-value">{{ $data['booking_created_at'] }}</div>
                </div>
                <div class="detail-row">
                    <div class="detail-label">تاريخ صلاحية الحجز:</div>
                ...
                <div class="detail-row">
                    <div class="detail-label">الحجز:</div>
            ...
            <div class="total-amount">
                ...
                <div class="total-value">{{ $data['price'] }} ج.م</div>
            </div>

            <div style="text-align: center;">
                <a href="{{ route('user.bookings.show', $bookings->id) }}" class="btn">عرض الفاتورة</a>
            </div>
        </div>
